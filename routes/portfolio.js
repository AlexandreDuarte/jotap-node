...
router.get('/imageoverlay', async function (req, res, next) {
  ...
  res.render('imageoverlay', { obras: rows[0] });
});
...
router.get('/', async function (req, res, next) {
  ...
  if (req.query.filter) {
    if (req.query.filter === "other") {
      ...
      res.render('portfolio', { obrass: rows, activefilter: [false, false, false, true] });
    } else {
      ...
      res.render('portfolio', { obrass: rows, activefilter: [false, qfilter === "canvas", qfilter === "mural", false] });
    }
    
  } else {
    const { rows } = await pool.query('SELECT * FROM obras ORDER BY year DESC, id DESC FETCH FIRST 5 ROW ONLY;');
    
    res.render('portfolio', { obrass: rows, activefilter: [true, false, false, false] });
  }
...
router.get('/griditems', async function (req, res, next) {
  ...
  if (req.query.filter) {
    if (req.query.filter === "other") {
      ...
      if (rows) {
        res.render('griditem-batch', { obrass: rows });
      } else {
        res.send('');
      }
    } else {
